#!/usr/bin/env python3
"""
Headless D435 face detection + XYZ with reduced CPU/D2H overhead:

Changes vs previous version:
- Do NOT copy or decode keypoints outputs (kps). We only use scores+boxes.
- Top-K pruning per feature level before decoding boxes + global NMS.
- Avoid per-frame astype() conversions on outputs (assumes float32 outputs).
- Keep two-class separation:
    - FaceDetectorTRTSCRFDBoxesOnly
    - RealSenseD435XYZ

Assumptions:
- TensorRT engine input is fixed (1,3,480,480) float32
- Outputs are 9 bindings like you saw; we only use (scores, boxes) for each level:
    level0: (1 scores, 2 boxes)   (skip 3 kps)
    level1: (4 scores, 5 boxes)   (skip 6 kps)
    level2: (7 scores, 8 boxes)   (skip 9 kps)
- SCRFD-style distances-to-bbox decoding, strides inferred from output length.
...
# Detection quality/speed tradeoffs
CONF_THRESH = 0.55           # raise to reduce candidates (speed up)
NMS_IOU_THRESH = 0.4
TOPK_PER_LEVEL = 400         # lower = faster, higher = more recall
STRIDES = (8, 16, 32)
...
class FaceDetectorTRTSCRFDBoxesOnly:
    ...
    Only copies/decodes scores+boxes; skips keypoints to reduce overhead.

    Input: full BGR frame (640x480)
    Internal: center-crop to 480x480, RGB, normalize [0,1], NCHW
    Output: detections in original full-frame coords (640x480)
    """
    ...
    def __init__(
        ...
        # (scores_idx, boxes_idx) for each level. kps bindings intentionally skipped.
    ...
    def _preprocess_center_crop(self, frame_bgr: np.ndarray):
        """
        Center-crop to model input, then BGR->RGB, normalize [0,1], NCHW float32.
        Returns (inp, x0, y0) where x0/y0 are crop offsets in original frame.
        """
    ...
    def _decode_level_topk(self, scores_2d: np.ndarray, boxes_2d: np.ndarray):
        """
        scores_2d: (N,1)
        boxes_2d:  (N,4) distances
        Returns decoded boxes (M,4) and scores (M,) in input-crop coords,
        with TOPK pruning applied before decode, and CONF_THRESH applied.
        """
        ...
        # Quick prune: if very few above threshold, only keep those; else topK
        above = np.where(scores >= self.conf_thresh)[0]
        if above.size == 0:
            return np.zeros((0, 4), np.float32), np.zeros((0,), np.float32)

        if above.size > self.topk_per_level:
            # topK within the above-threshold set
        ...
        centers = self._make_centers(fh, fw, stride, A)  # (N,2)

        sel_centers = centers[idx]
        sel_boxes = boxes_2d[idx] * stride  # distances -> pixels
    ...
    def detect(self, frame_bgr: np.ndarray) -> List[FaceDetection]:
        ...
        # D2H only scores+boxes (skip kps bindings)
        level_outputs = []
        for (si, bi) in self.output_groups:
            ...
            # reshape views; avoid astype copies
...
def main():
    ...
    det = FaceDetectorTRTSCRFDBoxesOnly(
        ...
        strides=STRIDES,
        output_groups=((1, 2), (4, 5), (7, 8)),  # skip kps bindings
    )
